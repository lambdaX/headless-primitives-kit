...
// Use aliased imports which are resolved via tsconfig.json paths for headless-logic modules
...
export function useHeadlessComponent<T extends HeadlessComponent<S>, S extends BaseComponentState>(
  ...
  useEffect(() => {
    ...
    const unsubscribeTransition = component.subscribe('stateTransition', () => {
        setComponentState(component.getState()); // Ensure latest state is pulled
        setCssState(component.getCSSState()); // Ensure latest CSS state is pulled
    });
    const unsubscribeHistory = component.subscribe('historyChanged', onHistoryChanged as any);
    
    // Initial sync in case state was modified in constructor or immediately after
